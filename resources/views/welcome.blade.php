    ...
    <div class="relative flex justify-center min-h-screen bg-gray-100 sm:items-center sm:pt-0">
        <div>
            ...
            <div class="flex justify-center">
                @auth
                <a href="{{ url('/dashboard') }}" class="text-sm text-gray-900">Дашборд</a>
                @else
                <a href="{{ route('login') }}" class="text-sm text-gray-900 hover:underline mr-3">Войти</a>

                @if (Route::has('register'))
                <a href="{{ route('register') }}" class="text-sm text-gray-900 hover:underline">Регистрация</a>
                @endif
                @endif
            </div>
            @endif
